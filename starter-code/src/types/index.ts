 ...
 * 
 * PRICING CONVENTIONS:
 * - `unit_price`: The price per unit per billing period (e.g., monthly rate)
 * - `total_price`: The total contract value for this line item
 * 
 * VALIDATION RULES:
 * - For `one_time` items: total_price = quantity × unit_price
 * - For recurring items (monthly/quarterly/annually): 
 *   total_price = quantity × unit_price × number_of_periods_in_contract
 *   Example: $1,000/month × 12 months = $12,000 total_price
 * 
 * The opportunity's `amount` should equal the sum of all line items' `total_price`.
 */
export interface LineItem {
  ...
  quantity: number;
  /** Price per unit per billing period (e.g., monthly rate for monthly billing) */
  unit_price: number;
  /** Total contract value for this line item (see pricing conventions above) */
  total_price: number;
...
/**
 * Opportunity represents a sales deal that needs billing actions.
 * 
 * VALIDATION RULES:
 * - `amount` must equal the sum of all line_items' total_price values
 * - `contract_end_date` must be after `contract_start_date`
 * - For conversion orders: `recurly_account_code`, `existing_self_service`, and `billing_transition` are required
 */
export interface Opportunity {
  ...
  account_name: string;
  account_id: string;
  /** Required for conversion_order types */
  recurly_account_code?: string;
  opportunity_name: string;
  close_date: string;
  /** Total contract value - must equal sum of all line_items.total_price */
  amount: number;
